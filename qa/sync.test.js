...
// DIAGONAL PREMIUM COVER (replaces vertical 57/43 per attached reference)
t('cover logo diagonal uses real assets/images/logo-white.png (inverted)', (()=>{ const img=d.querySelector('.cover-logo-diagonal img'); return img && (img.getAttribute('src')||'').includes('assets/images/logo-white.png'); })(), d.querySelector('.cover-logo-diagonal img')?.getAttribute('src'));
// gov line + benefits 4 (CLEANER/LOWER/RELIABLE/EXPERT) with circular orange icons
t('cover gov line Govt. Approved EPC Contractor with orange underline', (()=>{ const g=d.querySelector('.cover-gov'); return g && g.textContent.includes('Govt. Approved EPC Contractor'); })(), d.querySelector('.cover-gov')?.textContent);
t('cover benefits are exactly 4 with orange circular icons', (()=>{ const wrap=d.querySelector('.cover-benefits'); if(!wrap) return false; const items=wrap.querySelectorAll('.benefit'); if(items.length!==4) return false; const titles=[...items].map(b=>b.querySelector('.benefit-t')?.textContent.trim()); return titles.includes('CLEANER') && titles.includes('LOWER') && titles.includes('RELIABLE') && titles.includes('EXPERT') && [...wrap.querySelectorAll('.benefit-ico svg')].length===4; })(), d.querySelector('.cover-benefits')?.textContent);
...
// dedicated proposal block: A DEDICATED PROPOSAL FOR Mr. ABC Customer with Location/System Size/Proposal No./Date rows
t('cover dedicated proposal exists with customer + 4 rows', (()=>{ const ded=d.querySelector('.cover-dedicated'); if(!ded) return false; const label=(ded.querySelector('.cd-label')?.textContent||'').trim(); if(!label) return false; const name=(ded.querySelector('#v_coverCustName')?.textContent||'').trim(); if(!name || name==='—') return false; const rows=[...ded.querySelectorAll('.cd-row')].map(r=>r.textContent); return rows.length>=4 && rows.some(r=>r.includes('Location')) && rows.some(r=>r.includes('System Size')) && rows.some(r=>r.includes('Proposal No')) && rows.some(r=>r.includes('Date')); })(), d.querySelector('.cover-dedicated')?.innerHTML?.slice(0,400));
// diagonal bottom navy band 2 items + white circle icons with explicit width/height
t('cover bottom is navy band with 2 items + white circle icons with explicit width/height', (()=>{ const band=d.querySelector('.cover-bottom'); if(!band) return false; const items=band.querySelectorAll('.cb-bottom-item'); if(items.length!==2) return false; const labels=[...band.querySelectorAll('.cb-bottom-label')].map(l=>l.textContent.trim()); if(!(labels.includes('Project Capacity') && labels.includes('Projected 25-year savings'))) return false; const svgs=[...band.querySelectorAll('svg')]; if(svgs.length<2) return false; return svgs.every(svg=> svg.getAttribute('width') && svg.getAttribute('height') ); })(), d.querySelector('.cover-bottom')?.innerHTML?.slice(0,400));
t('cover badge Kwp + savings nodes exist', !!d.getElementById('v_coverBadgeKwp') && !!d.getElementById('v_coverBadgeGen'));
// CSS diagonal: clip-path polygon slanted is required for diagonal aesthetic; forbid only unsafe text tricks
t('cover CSS diagonal uses clip-path polygon + print-safe (no bg-clip text)', (()=>{ const css=fs.readFileSync(path.join(ROOT,'assets/css/app.css'),'utf8'); const s=css.indexOf('.page--cover'); const e=css.indexOf('/* ---------- 9.', s); const coverSection = s!==-1 && e!==-1 ? css.slice(s,e) : css.slice(s,s+9000); return /clip-path\s*:\s*polygon/i.test(coverSection) && !/background-clip\s*:\s*text/i.test(coverSection) && !/backdrop-filter/i.test(coverSection) && !/mix-blend/i.test(coverSection); })());
t('cover CSS accent is solid #F2811D (no gradient text)', (()=>{ const css=fs.readFileSync(path.join(ROOT,'assets/css/app.css'),'utf8'); return /\.accent\s*\{[^}]*#F2811D/i.test(css) && !/background-clip\s*:\s*text/i.test(css.slice(css.indexOf('.page--cover'), css.indexOf('/* ---------- 9.'))); })());
t('cover CSS diagonal split + hero pos 38% with orange top border', (()=>{ const css=fs.readFileSync(path.join(ROOT,'assets/css/app.css'),'utf8'); const s=css.indexOf('.page--cover'); const e=css.indexOf('/* ---------- 9.', s); const sec=s!==-1&&e!==-1?css.slice(s,e):css; return /cover-left-diagonal/i.test(sec) && /cover-right-diagonal/i.test(sec) && /clip-path\s*:\s*polygon/i.test(sec) && /object-position:\s*center\s*38%/i.test(css) && /border-top:\s*3px\s*solid/i.test(sec); })());
