...
(async () => {
  ...
  try {
    ...
    check('missing reports get WhatsApp request',await view.$$eval('#shareSimActions a',els=>els.length===1 && els[0].href.startsWith('https://wa.me/919309486769?text=')));
    check('request includes customer/reference with safe encoding',await view.$eval('#shareSimActions a',e=>new URL(e.href).searchParams.get('text').includes('AUDIT/7')));
    ...
    check('partial reports still request missing PVsyst',await view.$$eval('#shareSimActions a',els=>els.length===2 && els[1].textContent.includes('Request PVsyst')));
    await view.close();
