...
(function(root) {
  ...
  function renderGallery(s) {
    ...
    if (!card) return;
    const url = galleryUrl(s), status = $('galleryStatus');
    if (status) status.textContent = url ? 'QR destination: ' + url : 'QR hidden: enter a public HTTPS gallery URL (not a local proposal link).';
  ...
  function buildPowerPages(s) {
    const f=F.compute(s), url=galleryUrl(s);
  ...
  root.Experience={publicUrl,galleryUrl,buildPowerPages,sync,whenReady:()=>qrReady};
})(window);
